...
# 3) Global heading (LaTeX text, not HTML)
glue("\\section*{{The Words Most Distinctive of the 1830s and 1860s}}
When the decades are compared with each other.

") %>% knitr::asis_output()

# 4) Emit tables in a 2-column LaTeX layout
idx_pairs <- split(seq_along(gt_tables),
                   ceiling(seq_along(gt_tables) / 2))

walk(idx_pairs, function(ix) {
  left  <- as.character(gt_tables[[ix[1]]])
  right <- if (length(ix) == 2) as.character(gt_tables[[ix[2]]]) else ""
