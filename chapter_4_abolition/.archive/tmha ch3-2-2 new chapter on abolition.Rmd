...
```
# Chapter: Text Mining British Concepts in the 1833 Debates About the Abolition of Slavery
...
# merge the text from the debates with metadata and label 
debates_1830 <- hansard_1830 %>%  # create a new dataset from the text of the hansard debates
  left_join(debate_metadata_1830) %>% # merge the debates with metadata about debate title and speechdate
  mutate(year = year(speechdate)) # add a column for the year, taking the year information only from the complete speechdate

# find just the debates about slavery and who spoke them 
slavery_debates_1830 <- debates_1830 %>% # create a new dataset 
  filter(str_detect(tolower(debate), "slavery")) %>% # filter for the word 'slavery' in the title of the debates
  left_join(speaker_metadata_1830) %>% # merge the text/date dataset with data about the speakers 
  select(speaker, suggested_speaker, text, year) %>% # retain only certain columns 
  filter(suggested_speaker != "") # remove any lines of data missing speaker information
...
words_per_speaker_slavery_1830 <- slavery_words_1830 %>% # create a new dataset
  ...
  summarize(words_per_speaker = n()) %>% # create a new column -- words_spoken -- with total words per speaker for the entire time period
  arrange(desc(words_per_speaker)) # arrange the words in descending order from highest to smallest count
...
words_per_speaker_per_year_slavery_1830 <- slavery_words_1830 %>% # create a new dataset
  group_by(speaker, year) %>% # group the data by speaker and year 
  summarize(words_pspy = n()) %>% # create a new column -- words_pspy -- with the number of words per speaker per year 
  arrange(desc(words_pspy)) # arrange the words in descending order from highest to smallest count
...
top_words_pspy_slavery_1830 <- words_per_speaker_per_year_slavery_1830 %>%
  filter(speaker %in% unique(top_slavery_speakers$speaker)) # only retain those rows that correspond to the top speakers

# plot a chart of who spoke about slavery, how much, and when
ggplot(top_words_pspy_slavery_1830, # make a graph 
       aes(x = year, # the year is on the x axis
           y = words_pspy, # the y axis is the number of words spoken 
           label = speaker)) + # any labeling of dots that follows will use the 'speaker' field in the dataset
  geom_text_repel(size = 2) + # instructions to label each dot with the name of the corresponding speakers in a small font 
  geom_point() + # each speaker year/words spoken corresponds to a dot
  ggtitle("Who spoke about slavery, how much, and when?") + # label the title
  ylab("Number of words spoken") + # label the y axis 
  scale_x_continuous(breaks = function(x) unique(floor(pretty(seq(min(x), (max(x) + 1) * 1))))) + # use whole numbers, not decimals, for the x axis
  theme_light() # minimalist aesthetics
```
...
# New Chapter: From Concepts to ideas
...
tfidf_per_top_slavery_speaker_1833 <- top_words_per_abolition_speaker %>% # start with a dataframe with wordcounts per speaker
  bind_tf_idf(word, speaker, n) %>% # bind_tf_idf takes arguments 'term', 'document', and 'n'. In this case, the groupings of words we want to explore ('document') are the speakers
  arrange(desc(tf_idf))
...
speaker1 <- most_distinctive_words_per_abolition_speaker %>% filter(speaker == pattern1[1]) %>% select(word, tf_idf)
speaker2 <- most_distinctive_words_per_abolition_speaker %>% filter(speaker == pattern1[2]) %>% select(word, tf_idf)
speaker3 <- most_distinctive_words_per_abolition_speaker %>% filter(speaker == pattern1[3])%>% select(word, tf_idf)
speaker4 <- most_distinctive_words_per_abolition_speaker %>% filter(speaker == pattern1[4])%>% select(word, tf_idf)
speaker5 <- most_distinctive_words_per_abolition_speaker %>% filter(speaker == pattern1[5])%>% select(word, tf_idf)
```
...
most_distinctive_words_per_abolition_speaker %>%
  ...
  scale_y_continuous(breaks = function(x) unique(floor(pretty(seq(min(x), (max(x) + 1) * 1))))) + # use whole numbers, not  decimals, for the x axis +
...
buckingham_sentences <- slavery_debates_1833 %>%
  ...
  filter(speaker == "Mr. Buckingham") %>%
  filter(str_detect(text, "wages"))

# some fancy code to format the results as a table
kable(buckingham_sentences, format = "html", caption = "Buckingham's sentences about slavery invoking the word 'wages'") %>% 
...
knitr::kable(buck_speeches, format = "html", col.names = "", caption = "Three Speeches by Buckingham about Slavery That Invoke the Word 'Wages'") %>% 
    kable_styling(bootstrap_options = c("striped", "hover"))
...
```{r, message = F}
buckingham_sentences2 <- slavery_debates %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham") %>%
  filter(str_detect(text, "enjoyment"))
```
...
```{r, message = F}
buckingham_sentences3 <- slavery_debates %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham") %>%
...
buckingham_sentences4 <- slavery_debates %>%
  left_join(speaker_metadata_1830) %>%
  filter(speaker == "Mr. Buckingham") %>%
  filter(str_detect(text, "universal"))
```
...
```{r}
missing_context <- hansard_1830 %>% filter(sentence_id %in% c("S3V0018P0_8990", "S3V0018P0_8991", "S3V0018P0_8992", "S3V0018P0_8993"))
...
# make a table
kable(my_words_w_tfidf, format = "html", caption = "Some of Mr. Buckingham's Most Distinctive Words in the Debates About Slavery")
...
# some fancy code to turn it into a table
knitr::kable(peel_speeches, format = "html", col.names = "", caption = "Three Speeches by Buckingham about Slavery That Invoke the Word 'Wages'") %>% 
    kable_styling(bootstrap_options = c("striped", "hover"))
...
*** STEPH: I'd love your help in creating a basic collocate analysis of 'freedom' 1830-32 and 1834-36, and a more in-depth look at which words changed their collocates the most before and after the debates, just in debates of slavery. We could foreshadow word bectors here by doing a vector subtraction. What do you think?
