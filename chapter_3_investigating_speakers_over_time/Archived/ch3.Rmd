...
chancellor_of_the_exchequeur <- words_1830 %>%
  filter(str_detect(speaker, "Exchequer"),
         ...
         suggested_speaker != "", # remove if issue fixed
         suggested_speaker != "chancellor of the exchequerr") %>% # remove if issue fixed
...
chancellor_of_the_exchequeur %>% sample_n(5) %>% head()
```
...
hansard_1830_w_year %>% head() %>% select(speechdate, text, year) %>% kable() #look at the first few columns of the dataset
...
words_1830_w_year %>% head() %>% select(speaker, word, year) %>% kable() #look at the first few columns of the dataset
```
...
head(mrs_walrand_sentences) %>% kable()
```
