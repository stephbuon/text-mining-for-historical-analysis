...
In the previous chapters, we worked with intentionally small, subsetted datasets so the examples would run on a casual laptop. In this chapter, we take the opposite approach: we join multiple decades and create a much larger dataset first, and then filter it as needed. This allows us to demonstrate another workflow for working with large-scale historical data—one in which analysts begin with the full corpus and progressively narrow the scope through filtering and subsetting.
