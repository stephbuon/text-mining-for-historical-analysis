...
In previous chapters, we worked with datasets that were intentionally scaled down to work on most entry-level laptops. This chapter is an intentional exception.

Because we will ask the computer to store data from three decades at a time, *the following exercise may crash smaller computers.* If this happens, do not be alarmed. Power users of data sometimes need to upgrade their machines or work on university-provided servers to accomplish their research; you can make inquiries about using a more powerful computer with your IT services department. For the purposes of the rest of the chapter, you may regard the code that follows as an example of a process, recognizing how the tools we have previously used are deployed in what follows.

Users with more modern laptops may find that the code runs, but it runs extremely slowly. The following block of text, for example, may well take an hour to run on some machines. It is worthwhile; the results merit patience. 
...
hansard <- bind_rows(annotated_1820, annotated_1830) %>%
  bind_rows(annotated_1840)
```
...
# inspect the data
prereform_only_speakers %>% sample_n(10) %>% head()
```
...
speaker_key <- hansard %>%
  ...
  filter(!str_detect(speaker,"^\\s*[0-9]*\\s*$,-&%$#@;|'\"")) %>% # remove all numbers and punctuation
...
As Prof. Guldi has explained in *The Dangerous Art of Text Mining* (2023), different approaches to distinctiveness can be used to explore the multiple dimensions of temporal change.  A skilled digital historian will refuse to settle for one set of answers about distinctiveness, instead choosing to explore many metrics.

If tf-idf is a tool for identifying the characterstics of an orange that can't be found in apples, another approach -- log likelihood -- helps us see the nuances that overlap. The mathematical features of this approach and their implications are explored in *The Dangerous Art of Text Mining,* so here, we will emphasize instead how to employ log likelihood to create an alternative comparison of two time periods.   

The *dhmeasures* package contains the *log_likelihood()* function, which acts much like *bind_tf_idf()*, accepting a dataframe with columns *word*, *group*, and *n*, and producing a score of distinctiveness ("ll") for each word-group pair. 

#***************Steph:Funny results here. I think the issue is the log_likelihood() function -- I expect one ll score for two input vectors. but we're getting two LL scores back. It's just inelegant.  I need a version that binds the ll score -- or I need help reworking the code below to use the current output.  We can delete if needs be.
...
#*** Steph, is it possible to modify this so that we're only looking at the House of Commons? People who really care about parliament will want this information. If you do it, adjust this paragraph and the code throughout so that we're not analyzing speeches from the House of Lords, only the Commons.


#*** Steph, can you eliminate the NA scores that show up in the visualization below?
...
Again, a list of words emerges which is hard to interpret. In this case, we choose to ask the analyst to supply their own interpretation in the exercises for this chapter. 
...
One of the amazing things about working with data is the ease of repeating the same analysis on new data. If we're content with what we found about 1832, all we have to do to analyze 1867 is apply the same analysis to newer data. 

Users should note that the number of speeches given every year expanded dramatically over the century, with the result that the following code may be even slower than the previous section. 
...
#***** STeph - not working -- attempt to model top speakers against others -- no new results
```{r}
library(textstem)
...
2) This chapter leverages a number of applications of distinctiveness measures -- like tf-idf and log likelihood -- to ask, in different ways, what is special about the speakers before and after the Reform Act? In your own words, explain what was done with the data in each of these exercises: 
  a) tf-idf by time period
  b) log likelihood by time period
  c) comparing speakers from one period only against those continuously elected
  d) comparing the most prolific speakers who were exclusively prereform against the most prolific speakers who were exclusively postreform

3) The approach profiled here generated many analyses filled with obscure  names, in part because we have used the stricter tf-idf rather than the more inclusive log likelihood measure.  For the analyses of prereform and post reform speakers and their words, change the code so that the analysis uses log likelihood. Are the results any easier to interpret?

4) We have generated an analysis of 1832 and of 1867. Choose your favorite of the visualizations, and consult the version of this analysis for each event. What is the same in both events? What is different?  

5) We have not supplied an interpretation for all of the obscure place names that occur in the comparison of speakers elected in one period versus those elected continuously. Choose one of the charts, and use your skills to navigate back to in-text mentions of these words. Based on what you have found, give an interpretation of the significance of distinctiveness measures for our understanding of the Reform Act.  

6) In an essay of a page, explain why it is important to compare the results of different analyses rather than to choose one of these approaches at random. What do we get out comparing the results of different methods of analysis? Do you get more insight out of comparing different events (1832 vs 1867), or different aspects of the same event?
